...
} from "@/components/problem/ProblemFilters";
import { useStore } from "@/lib/store";
import type { LeetcodeTopic } from "@/lib/domain/types";

const DEFAULT: ProblemFilterState = {
  query: "",
  topic: "All",
  difficulty: "All",
  status: "All",
...
export default function ProblemListPage() {
  ...
  const [filters, setFilters] = useState<ProblemFilterState>(DEFAULT);

  const topics = useMemo(() => {
    const set = new Set<LeetcodeTopic>();
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">LeetCode Blind 75</h1>
        <p className="text-sm text-muted-foreground">
          週ごとのトピック × ステータスで管理する問題リスト。クリックで演習画面へ。
        </p>
      </div>

      <ProblemFilters value={filters} onChange={setFilters} topics={topics} />

      {filtered.length === 0 ? (
        <EmptyState title="該当する問題なし" description="フィルタ条件を緩めてください。" />
      ) : (
            ...
            {filtered.map((p) => (
              <li key={p.id}>
                ...
                >
                  ...
                  <div className="col-span-2 text-xs text-muted-foreground">{p.topic}</div>
