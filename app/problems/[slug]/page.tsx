...
import type { Problem, ProblemStatus } from "@/lib/domain/types";
...
export default function ProblemWorkbenchPage() {
  ...
  const patch = useCallback(
    (k: keyof Problem) => (v: string) => slug && update(slug, { [k]: v } as Partial<Problem>),
    [slug, update],
  );
  ...
  if (!problem) {
    return (
      <div className="mx-auto max-w-3xl py-10 text-center">
        ...
        <Link href="/problems" className="mt-3 inline-block text-sm text-primary underline">
          一覧に戻る
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        ...
        <Card className="xl:col-span-1">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">検証 / レビュー</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <NoteField
              label="Edge cases"
